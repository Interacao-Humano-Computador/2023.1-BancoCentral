...
# Verificação do artefato do Cronograma Planejado do Grupo 4 (Open Street Map)
...
| O artefato apresenta as datas de início e fim de cada atividade? | Sim | Excelente | Todas as datas de desenvolvimento estão presentes no arquivo. |
| O artefato apresenta um período destinado a revisão dos artefatos entreges? | Sim | Excelente | Todas as datas de revisão estão presentes no arquivo. |
| O artefato apresenta um período destinado a gravação das entregas para cada ponto de controle? | Sim | Excelente | As gravações estão previstas no documento. |
| O artefato prevê quais os métodos de análise de tarefas serão utilizados? | Não | - | O documento não prevê quais técnicas de análise serão utilizadas no desenvolvimento do projeto. |
...
Embora o artefato esteja, em sua maior parte, completo em relação às expectativas, algumas pequenas ressalvas são necessárias:
  ...
  - O artefato deveria detalhar melhor qual o objetivo do documento em si, visto que não há nenhum texto sobre o motivo da existência do arquivo.
  - O artefato deveria prever as técnicas de análise de tarefas as quais forem utilizadas no projeto.
  - Existem erros a cerca da ordem da entrega de alguns artefatos previstos no cronograma. Ex: a entrega 2 prevê a entrega do arquivo de análise de tarefas.
  </div>
...
Algumas correções sugeridas são:
  ...
  - Chamar as todas as tabelas em texto.
  </div>
